...
func (tq *TaskQuery) QueryAssignments() *TaskAssignmentQuery {
	query := (&TaskAssignmentClient{config: tq.config}).Query()
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withAssignments: tq.withAssignments.Clone(),
...
func (tq *TaskQuery) WithAssignments(opts ...func(*TaskAssignmentQuery)) *TaskQuery {
	query := (&TaskAssignmentClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAssignments = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withAssignments; query != nil {
		if err := tq.loadAssignments(ctx, query, nodes,
...
func (tq *TaskQuery) loadAssignments(ctx context.Context, query *TaskAssignmentQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskAssignment)) error {
