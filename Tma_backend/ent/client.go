...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.TaskAssignment.Intercept(interceptors...)
...
// UpdateOneID returns an update builder for the given id.
func (c *TaskAssignmentClient) UpdateOneID(id uint64) *TaskAssignmentUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *TaskAssignmentClient) DeleteOneID(id uint64) *TaskAssignmentDeleteOne {
...
func (c *TaskAssignmentClient) Get(ctx context.Context, id uint64) (*TaskAssignment, error) {
	return c.Query().Where(taskassignment.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *TaskAssignmentClient) GetX(ctx context.Context, id uint64) *TaskAssignment {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
func (c *UserClient) QueryAssignments(u *User) *TaskAssignmentQuery {
	query := (&TaskAssignmentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Task, TaskAssignment, User []ent.Hook
	}
	inters struct {
		Task, TaskAssignment, User []ent.Interceptor
	}
)
